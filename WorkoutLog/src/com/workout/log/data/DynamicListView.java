   ...
   public OnItemLongClickListener getItemLongClickListener(){
    ...
    private void handleCellSwitch() {
        ...
        if (isBelow || isAbove) {
            ...
            if(isBelow){
            	success = swapElements(originalItem, getPositionForView(switchView), false);
            }else{
            	success = swapElements(originalItem, getPositionForView(switchView), true);
            }   
    ...
    private boolean swapElements(int indexOne, int indexTwo, boolean isUp) {
    	ListItem swapItem = itemList.get(indexOne);
        ListItem swapToItem = itemList.get(indexTwo);
        
        int startId = indexOne;
        int swapId = indexTwo;
        
        if (swapToItem.isSection()){
        	if(isUp){	        		
        		/**
        		 * Increase to overstep the header
        		 */
        		swapId--;
        		
        		if (swapId > 1){
	        		/**
	        		 * Add Header below swapToItem, Rename old Header to Exercise Musclegroup
	        		 */
	        		if (swapId-1 < itemList.size()){
		        		if (!itemList.get(startId+1).isSection()){
		        			itemList.add(startId+1, new MuscleGroupSectionItem(((MuscleGroupSectionItem) swapToItem).getTitle()));
		        		}
	        		}
	        		((MuscleGroupSectionItem)itemList.get(startId-1)).setTitle(((Exercise) itemList.get(swapId)).getMuscleGroup().getName());
	        		
	        		/**
	        		 * if exercise is the last one in the Musclegroup then delete the group
	        		 */
	        		if (itemList.get(swapId-1).isSection()){
	        			itemList.remove(swapId-1);
	        			startId--;
	        			swapId--;
	        		}
	        		 ...
	        		 * Make the swap
	        		 */
	        		itemList.set(startId, itemList.get(swapId));
	   		        itemList.set(swapId, swapItem);
	   		        
	   		        /**
	   		         * Add new header above indexOne
	   		         */
	        		itemList.add(swapId, new MuscleGroupSectionItem(((Exercise) itemList.get(swapId)).getMuscleGroup().getName()));
	        		
	        		/**
	        		 * To ensure that the count is still on the same Item
	        		 */
	        		swapId++;
	        		startId++;
	        		
	        		/**
	        		 * Merge Musclegroups down
	        		 */
	        		if (startId+1 < itemList.size()){
		        		if (itemList.get(startId+1).isSection()){
		        			if(((MuscleGroupSectionItem)itemList.get(startId+1)).getTitle().equals(((Exercise) itemList.get(startId)).getMuscleGroup().getName())){
		        				itemList.remove(startId+1);
		        			}
		        		}
	        		}
	        		/**
	        		 * Merge Musclegroups up
	        		 */
	        		if (itemList.get(swapId-1).isSection()){
	        			if (swapId-2 >= 0){
		        			if(((MuscleGroupSectionItem)itemList.get(swapId-1)).getTitle().equals(((Exercise) itemList.get(swapId-2)).getMuscleGroup().getName())){
		        				itemList.remove(swapId-1);
		        			}
	        			}
	        			return true;
        		...
        		int tempIndex = swapId;
        		
        		/**
        		 * Increase to overstep the header
        		 */
        		swapId++;
        		if (swapId <= itemList.size()){
	        		/**
	        		 * Add Header below swapToItem, Rename old Header to Exercise Musclegroup
	        		 */
	        		if (swapId+1 < itemList.size()){
		        		if (!itemList.get(swapId+1).isSection()){
		        			itemList.add(swapId+1, new MuscleGroupSectionItem(((MuscleGroupSectionItem) swapToItem).getTitle()));
		        		}
	        		}
	        		((MuscleGroupSectionItem)itemList.get(tempIndex) ).setTitle(((Exercise) swapItem).getMuscleGroup().getName());
	        		...
	        		itemList.set(startId, itemList.get(swapId));
	   		         ...
	   		         * Add new header above indexOne
	   		         */
	        		itemList.add(startId, new MuscleGroupSectionItem(((Exercise) itemList.get(startId)).getMuscleGroup().getName()));
	        		...
	        		if (swapId+1 < itemList.size()){
		        		if (itemList.get(swapId+1).isSection()){
		        			if(((MuscleGroupSectionItem)itemList.get(swapId+1)).getTitle().equals(((Exercise) itemList.get(swapId)).getMuscleGroup().getName())){
		        				itemList.remove(swapId+1);
		        			}
		        		}
	        		}
	        		...
	        		if (itemList.get(startId-1).isSection()){
	        			if (startId-2 >= 0){
		        			if(((MuscleGroupSectionItem)itemList.get(startId-1)).getTitle().equals(((Exercise) itemList.get(startId-2)).getMuscleGroup().getName())){
		        				itemList.remove(startId-1);
		        			}
	        			}
	        		}
